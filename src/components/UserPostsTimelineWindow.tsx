...
const UserPostsTimelineWindow: React.FC<UserPostsTimelineWindowProps> = ({
  id,
  userId,
  userAcct,
  onImageClick,
  ...
  const fetchUserPosts = async (loadMore = false) => {
    ...
    try {
      ...
      const response = await fetch(`${snap.serverUrl}/api/v1/accounts/${userId}/statuses?${params}`, {
        headers: {
      ...
      setTimeout(() => {
        updatePostEngagementCounts(newStatuses, `User Posts (@${userAcct})`);
      }, 100);
  ...
  const getWindowTitle = () => {
    const title = `@${userAcct} posts`;
    // Truncate if too long (keeping it under 30 characters for window title bar)
    if (title.length > 30) {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ color: '#808080', fontSize: '11px' }}>
            {statuses.length} posts by @{userAcct}
          </span>
        </div>

        {/* Content area */}
        <div 
          data-window-type={`User Posts (@${userAcct})`}
          style={{
            flex: 1,
            overflow: 'auto',
            padding: '8px',
...
export default UserPostsTimelineWindow;